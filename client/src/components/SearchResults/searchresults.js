...
const SearchResults = ({housingData,loggedInUser}) => {
  ...
  const handleFavoriteToggle = async (aptId) => {
    try {
      if (isFavorite(aptId)) {
        await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/user/favorites/remove`, {
          method: 'DELETE',
        ...
        await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/user/favorites/add`, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            username: loggedInUser.email,
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      if (loggedInUser) {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/user/favorites?username=${loggedInUser.email}`);
  ...
  const handleCommentSubmit = (e) => {
    ...
    if (comment.trim()) {
      ...
      fetch(`${process.env.REACT_APP_BACKEND_URL}/api/comments/add`, {
        method: 'POST',
        headers: {
